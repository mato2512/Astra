<script lang="ts">
	...
	// Remove emojis to prevent TTS from reading descriptions like "smiling face with smiling eyes"
	content = content.replace(/[\u{1F600}-\u{1F64F}\u{1F300}-\u{1F5FF}\u{1F680}-\u{1F6FF}\u{1F1E0}-\u{1F1FF}\u{2702}-\u{27B0}\u{24C2}-\u{1F251}\u{1F900}-\u{1F9FF}\u{1FA00}-\u{1FA6F}\u{1FA70}-\u{1FAFF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}]+/gu, '').replace(/\s+/g, ' ').trim();		if ($config.audio.tts.engine === '') {
			let voices = [];
		...
		<div class="flex-auto w-0 pl-1 relative">
			...
			<div>
				...
				{#if !edit}
					...
					{#if $user?.role === 'admin' && (isLastMessage || ($settings?.keepFollowUpPrompts ?? false)) && message.done && !readOnly && (message?.followUps ?? []).length > 0}
